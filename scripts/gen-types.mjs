  ...
  JSON.stringify(
    {
      compilerOptions: {
        ...
        noEmit: false,
        // Node types: src/worker/node.ts (the Node adapter) resolves
        // node:worker_threads / node:path etc. The emitted JS keeps the
        // specifiers; consumers with a DOM-only tsconfig never compile
        // this file (the "node" export condition routes them away).
        types: ["node"],
      ...
      files: [
        ...
        join(repoRoot, "src", "raw", "index.ts"),
        // M5 worker entries: the browser client, the shared session, the
...
// Step 4 (M5): the worker entry. The browser client (emitted at
// dist/types/worker/index.js) resolves its worker through
// new URL('./worker.mjs', import.meta.url) — that pattern only works
// when the entry is a SIBLING of the client, because bundlers rewrite
// the URL to an emitted chunk relative to the importing file. A tiny
// re-export shim breaks under bundler inlining (vite inlines files
// below its asset size limit as data: URLs, where relative imports
// cannot resolve), so the entry is the FULL boot code: a copy of the
// emitted worker.js under the .mjs name. Its relative imports
// (../core/types.js etc.) resolve identically to worker.js's own.
// dist/worker.mjs remains as the stable top-level entry the Node
// adapter probes for and direct spawners can target (design §5).
const workerDir = join(outDir, "worker");
mkdirSync(workerDir, { recursive: true });
const workerJs = readFileSync(join(workerDir, "worker.js"), "utf8");
// The wasm loader import is a bare self-referencing specifier in src
// (Node resolution honors it; the loader only exists post-build).
// Bundlers following the dist layout need a relative path — rewrite it
// in BOTH emitted worker files (worker.js is reached through the
// top-level dist/worker.mjs alias; worker.mjs is the browser client's
// sibling entry). From dist/types/worker/, ../../leptonica.mjs is
// dist/leptonica.mjs in every install layout (the path stays inside
// the package — pnpm's realpath store included).
const distWorkerJs = workerJs.replaceAll(
  '"leptonica-wasm/leptonica.mjs"',
  '"../../leptonica.mjs"',
);
writeFileSync(join(workerDir, "worker.js"), distWorkerJs);
const banner = "// Generated by gen-types.mjs - worker entry (copy of worker.js; see Step 4).\n";
writeFileSync(join(workerDir, "worker.mjs"), banner + distWorkerJs);
writeFileSync(join(repoRoot, "dist", "worker.mjs"), [
