...
import { assertSourceIdentityUnchanged, resolveSourceIdentity } from "./source-identity.mjs";

const repoRoot = dirname(dirname(fileURLToPath(import.meta.url)));
const expectedSourceCommit = process.env.LEPTONICA_WASM_SOURCE_COMMIT;
const sourceIdentityBeforeBuild = resolveSourceIdentity(repoRoot, expectedSourceCommit);
...
const sourceIdentity = resolveSourceIdentity(repoRoot, expectedSourceCommit);
assertSourceIdentityUnchanged(sourceIdentityBeforeBuild, sourceIdentity);
const provenance = {
  schemaVersion: 1,
