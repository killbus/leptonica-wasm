...
def test_source_workflow_matches_dispatch_contract() -> None:
    workflow = (
    ...
    assert (
        "BUILDER_DISPATCH_TOKEN: ${{ secrets.LEPTONICA_BUILDER_DISPATCH_TOKEN }}"
        in dispatch
    )
    assert "LEPTONICA_BUILDER_REPOSITORY must be owner/repository" in dispatch
    assert "LEPTONICA_BUILDER_DISPATCH_TOKEN is not configured" in dispatch
    ...
    assert "curl_status -ne 0" in dispatch
    assert '"$status" == "429"' in dispatch
    assert '"$status" =~ ^5[0-9][0-9]$' in dispatch
    assert "retrying in ${delay}s" in dispatch
    assert "delay=$((delay * 2))" in dispatch
    assert "Non-retryable builder dispatch failure" in dispatch
...
if __name__ == "__main__":
    ...
    test_source_workflow_matches_dispatch_contract()
